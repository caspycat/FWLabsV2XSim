...
configurationFile = fullfile( ...
    exampleDirectory,"13_coexistence_methods.toml");
...
simulationResults = cell(numel(methods),1);
...
for index = 1:numel(methods)
    ...
    patch = v2xsim.config.patch(struct(Coexistence=coexistence));
    configuration = template.resolve(Patch=patch);
    simulationResult = v2xsim.runSimulation( ...
        configuration, ...
        OutputDirectory=fullfile(outputDirectory,method), ...
        RunLabel=method);
