...
% route perception, controller-bound packet loss, and fixed transport delay
% on the exit-ramp scenario.
...
for fileIndex = 1:numel(traceFiles)
    traceFile = fullfile( ...
        traceFiles(fileIndex).folder,traceFiles(fileIndex).name);
    importOptions = detectImportOptions(traceFile, TextType="string");
    importOptions = setvartype( ...
        importOptions, ...
        ["VehicleId","TrueRoute","ModuleType","StatusEffectType", ...
         "LifecycleEvent","NetworkUpdateOutcome"], ...
        "string");
    rows = readtable(traceFile,importOptions);
    if isempty(trace)
        trace = rows;
    else
...
networkUpdateRowCount = zeros(numel(moduleIndices),1);
droppedUpdateRowCount = zeros(numel(moduleIndices),1);
for moduleRow = 1:numel(moduleIndices)
    ...
    networkOutcome = trace.NetworkUpdateOutcome(mask);
    networkOutcome(ismissing(networkOutcome)) = "";
    networkUpdateRowCount(moduleRow) = sum( ...
        networkOutcome ~= "");
    droppedUpdateRowCount(moduleRow) = sum( ...
        networkOutcome == "Dropped");
end
moduleTable = table( ...
    moduleIndices,moduleType,statusEffectType,rowCount,activeRowCount, ...
    networkUpdateRowCount,droppedUpdateRowCount, ...
    VariableNames=[ ...
        "ModuleIndex","ModuleType","StatusEffectType", ...
        "TraceRowCount","ActiveRowCount","NetworkUpdateRowCount", ...
        "DroppedUpdateRowCount"]);

configuredModules = summary.Configuration.Positioning.ErrorChain.Modules;
configuredModuleCount = numel(configuredModules);
assert(configuredModuleCount == 5, ...
    "The summary did not retain the five configured modules.");
