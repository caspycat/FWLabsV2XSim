...
6. The simulation files have been adapted to the current explicit highway-scenario inputs and the independent `ratioSelectedAutonomousMode`, `ratioSelectedL2`, and `L2active` controls. The publication-era originals remain available at commit `720080210ada63a7e752809f2f936e74783b4d31`.
7. Where the archived configuration differs from the journal, the setting that reproduces the published curve or supports its conclusion is authoritative. The calibrated campaigns retain the executable configuration's 6 dB receiver noise figure and fixed 200 ms Figure 11 generation interval; the journal reports 9 dB and describes motion-driven ETSI CAM intervals averaging approximately 207 ms.
