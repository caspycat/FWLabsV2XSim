...
while timeManagement.timeNow < simParams.simulationTime
    ...
    elseif timeEvent == timeManagement.timeNextCBRupdate && ~delayCBRupdate
        ...
        if ~isempty(stationManagement.activeIDs11p)
            ...
            [timeManagement,stationManagement,cbrValues] = ...
                cbrUpdate11p(timeManagement,vehiclesToConsider,stationManagement,simParams,phyParams,outParams);
            stationManagement.cbr11pValues( ...
                vehiclesToConsider, ...
                ceil(timeEvent/simParams.cbrSensingInterval-1e-9)) = ...
                cbrValues;
            cbrInvocation = v2xsim.hook.invocations. ...
                AfterChannelBusyRatioSampledInvocation( ...
                    timeManagement.timeNow, "11p", ...
                    vehiclesToConsider(:), ...
                    stationManagement.vehicleChannel( ...
                        vehiclesToConsider), ...
                    cbrValues(:));
...
%             %% =========
        ...
        if ~isempty(stationManagement.activeIDsCV2X)
            ...
            [timeManagement,stationManagement,sinrManagement, ...
                cbrValues,technologyOnlyCbrValues] = ...
                cbrUpdateCV2X(timeManagement,vehiclesToConsider,stationManagement,positionManagement,sinrManagement,appParams,simParams,phyParams,outParams,outputValues,simValues);
            cbrWindowIndex = ceil( ...
                timeEvent/simParams.cbrSensingInterval);
            stationManagement.cbrCV2Xvalues( ...
                vehiclesToConsider,cbrWindowIndex) = cbrValues;
            stationManagement.coex_cbrLteOnlyValues( ...
                vehiclesToConsider,cbrWindowIndex) = ...
                technologyOnlyCbrValues;
            cbrInvocation = v2xsim.hook.invocations. ...
                AfterChannelBusyRatioSampledInvocation( ...
    ...
    elseif abs(timeEvent-timeManagement.timeNextPacket(idEvent))<1e-8   % timeEvent == timeManagement.timeNextPacket(idEvent)
        ...
        if stationManagement.vehicleState(idEvent)==constants.V_STATE_LTE_TXRX % is LTE
            ...
            [timeManagement,stationManagement,sinrManagement,outputValues] = ...
                newPacketIn11p(idEvent,indexEvent,simParams,positionManagement,...
                phyParams,timeManagement,stationManagement,sinrManagement,outputValues,simValues);
