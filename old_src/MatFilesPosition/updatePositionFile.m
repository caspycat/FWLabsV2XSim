function [XvehicleReal,YvehicleReal,IDvehicle,indexNewVehicles,indexOldVehicles,indexOldVehiclesToOld,IDvehicleExit,speedNow, direction] = updatePositionFile(time,dataTrace,oldIDvehicle,XvehiclePrevious,YvehiclePrevious,timePrevious,simValues,~)
...
vehicleCount = numel(IDvehicle);
unknownKinematics = NaN(vehicleCount,1);
vehicleKinematics = table( ...
    ...
    RowNames=cellstr(string(IDvehicle(:))));
dispatchAfterVehicleKinematicsUpdated( ...
    simValues,time,vehicleKinematics);
